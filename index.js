...
game={
	...
	say_word:async function (word) {
		...
		let spk=await EasySpeech.speak({
		  text: word,
		  voice: ru_voices[irnd(0,ru_voices.length-1)], // optional, will use a default or fallback
		  pitch: 1,
		  rate: 1,
		  volume: 1,
...
async function init_game_env(lang) {
	...
	console.log(res);
	all_voices = EasySpeech.voices()
	console.log("All Voices: "+all_voices.length)
	
	ru_voices = all_voices.filter(function (el) {
	  return ['ru-RU','ru_RU'].includes(el.lang)
	});
	
	console.log("Ru Voices: "+ru_voices.length)
